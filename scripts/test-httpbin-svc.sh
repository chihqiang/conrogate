...
#
# 用法：
#   ./scripts/test-httpbin-svc.sh           # 创建上游/路由并发布、验证
#   ./scripts/test-httpbin-svc.sh --cleanup # 删除示例上游与路由（需 --keep-config 先创建）
#
# 可覆盖的环境变量：
...
cleanup() {
  ...
  [ "$deleted" = "1" ] || echo "  [SKIP] 未找到上游 $UPSTREAM_NAME"
  echo "  [提示] 路由随上游删除级联移除；需重新发布使配置生效"
...
echo "== 4. 发布配置 =="
pub_resp=$(curl -sS -X POST "${BASE}/configs/publish?remark=httpbin%20route" ${AUTH[@]+"${AUTH[@]}"})
...
echo "  清理示例配置：./scripts/test-httpbin-svc.sh --cleanup"
