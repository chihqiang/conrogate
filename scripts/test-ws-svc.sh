...
# WebSocket 接入测试：用 scripts/ws.php（Swoole）起本地 WS echo 上游，
...
#
# 用法：
#   ./scripts/test-ws-svc.sh            # 起上游 → 注册 → 发布 → 网关 WS 回显校验
#   ./scripts/test-ws-svc.sh --cleanup  # 删除示例上游与路由
#
# 可覆盖的环境变量：
...
WS_SERVER="$(cd "$(dirname "$0")" && pwd)/ws.php"   # Swoole：服务器 + 客户端回显校验
WS_HOST="127.0.0.1"
WS_PORT=""
WS_PID=""
WS_LOG=""
...
echo "  清理示例配置：./scripts/test-ws-svc.sh --cleanup"
