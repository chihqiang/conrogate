...
use conrogate_core::gateway::TelemetryReport;
use std::sync::Arc;
use tokio::sync::mpsc;
...
pub struct MetricAggregator {
    ...
    bucket_sec: u32,
    /// 落库通道
    metric_repo: Option<Arc<dyn conrogate_core::storage::MetricRepo>>,
    /// 落库失败的指标批次环形缓冲（指数退避重试兜底）
    recently_failed: std::collections::VecDeque<Vec<conrogate_core::dto::MetricRow>>,
}
...
impl MetricAggregator {
    ...
    /// 设置落库仓储
    pub fn with_metric_repo(mut self, repo: Arc<dyn conrogate_core::storage::MetricRepo>) -> Self {
