...
use conrogate_core::contract::storage::{ReadOnlyUpstreamRepo, UpstreamRepo};
...
impl ReadOnlyUpstreamRepo for UpstreamRepoImpl {
    ...
    async fn find_by_route(&self, route_id: u64) -> Result<Option<UpstreamDto>, ConrogateError> {
        ...
        let route = crate::entity::routes::Entity::find_by_id(route_id as i64)
            .filter(crate::entity::routes::Column::DeletedAt.is_null())
            .one(&self.db)
            .await
            .map_err(|_| ConrogateError::DatabaseInternal)?;
...
impl UpstreamRepo for UpstreamRepoImpl {
    ...
    async fn update(&self, dto: UpdateUpstreamDto) -> Result<UpstreamDto, ConrogateError> {
        ...
        if let Some(algo) = dto.algorithm {
            active.algorithm = Set(crate::convert::algorithm_to_i16(algo));
        }
    ...
    async fn list_route_bindings(
        ...
        let models = crate::entity::routes::Entity::find()
            .filter(crate::entity::routes::Column::UpstreamId.eq(id as i64))
            .filter(crate::entity::routes::Column::DeletedAt.is_null())
            .all(&self.db)
            .await
            .map_err(|_| ConrogateError::DatabaseInternal)?;
