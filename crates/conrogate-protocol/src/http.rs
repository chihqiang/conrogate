...
impl HttpProtocolHandler {
    ...
    async fn preflight(
        ...
        if crate::upgrade::is_upgrade_request(method, headers) {
            ...
            let host_value = route
                .host_header
                .clone()
                .unwrap_or_else(|| crate::proxy::upstream_host(&node).into());
    ...
    fn build_out_headers(
        ...
        let host_value: String = route
            ...
            .as_deref()
            .map(ToString::to_string)
            .unwrap_or_else(|| crate::proxy::upstream_host(node));
        if let Ok(v) = host_value.parse() {
...
mod tests {
    ...
    fn make_handler_inner(
        ...
        let svc = Arc::new(ServiceContext {
            ...
            blacklist: Arc::new(conrogate_security::blacklist::BlacklistMatcher::new()),
