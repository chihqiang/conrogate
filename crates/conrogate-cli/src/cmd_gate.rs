...
async fn async_run(config: conrogate_core::contract::config::Config) -> anyhow::Result<()> {
    ...
    let server = conrogate_gateway::server::GatewayServer::from_config_with_db(
        config.clone(),
        read_db,
        official_plugins(),
    )
...
async fn run_without_db(config: conrogate_core::contract::config::Config) -> anyhow::Result<()> {
    ...
    let server = Arc::new(
        conrogate_gateway::server::GatewayServer::from_config(config, official_plugins()).await,
    );
