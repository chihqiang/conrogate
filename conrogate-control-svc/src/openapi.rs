...
use utoipa::openapi::path::{
    HttpMethod, Operation, OperationBuilder, Parameter, ParameterBuilder, ParameterIn, PathItemBuilder,
    Paths, PathsBuilder,
};
...
fn bad_response() -> Response {
    ResponseBuilder::new()
        ...
        .content("application/json", json_content(Some(ref_to("ApiResponse"))))
        .build()
}
...
fn op(
    ...
    if let Some((schema_name, required)) = body {
        let request_body = RequestBodyBuilder::new()
            ...
            .required(Some(if required { Required::True } else { Required::False }))
            .content(
                "application/json",
                json_content(Some(ref_to(schema_name))),
            )
            .build();
...
fn build_paths() -> Paths {
    ...
    b = b
        .path(
            "/health",
            PathItemBuilder::new()
                .operation(HttpMethod::Get, op("health", "健康检查", "health_check", vec![], None))
                .build(),
        )
        .path(
            "/healthz",
            PathItemBuilder::new()
                .operation(HttpMethod::Get, op("health", "存活探针", "healthz", vec![], None))
                .build(),
        )
        .path(
            "/readyz",
            PathItemBuilder::new()
                .operation(HttpMethod::Get, op("health", "就绪探针", "readyz", vec![], None))
                .build(),
        );
    ...
    b = b
        ...
        .path(
            ...
            PathItemBuilder::new()
                ...
                .operation(HttpMethod::Delete, op(
                    "routes",
                    "删除路由",
                    "delete_route",
                    vec![id_param()],
...
fn build_components() -> Components {
    ComponentsBuilder::new()
        ...
        // 分页结果（泛型实例）
        .schema("PaginatedResult", PaginatedResult::<serde_json::Value>::schema())
        .build()
}
...
pub fn build_openapi() -> utoipa::openapi::OpenApi {
    let info = InfoBuilder::new()
        ...
        .version("0.1.0")
        .description(Some(
            "Conrogate 网关控制面 REST API — 路由/上游/插件/配置版本/指标/事件/审计/数据上报".to_string(),
        ))
        .build();
...
mod tests {
    ...
    fn openapi_has_paths_and_schemas() {
        ...
        let components = doc.components.as_ref().expect("openapi must have components");
        ...
        // 关键端点必须存在
        for p in ["/routes", "/routes/{id}", "/reports/metrics", "/insights/events"] {
