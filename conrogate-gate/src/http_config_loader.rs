...
use conrogate_contract::dto::{PluginBindingDto, RouteDto, UpstreamDto};
...
impl HttpConfigLoader {
    ...
    /// 拉取路由的插件绑定
    pub async fn load_plugin_bindings(
        &self,
        route_id: u64,
        ...
        serde_json::from_value(data.clone()).map_err(|e| {
            ConrogateError::ConfigLoad(format!("plugin bindings deserialize failed: {e}"))
        })
    }
